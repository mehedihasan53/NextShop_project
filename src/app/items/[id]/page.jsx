...
export async function generateMetadata({ params }) {
    ...
    return {
        title: 'Product Details - NextShop',
...
export default async function ItemPage({ params }) {
    ...
    // If no item found, show not found
    if (!item) {
        return <ProductNotFound />;
    }
        ...
        <div className="min-h-screen bg-slate-900">
            ...
            <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-16">
                ...
                <div className="mb-8">
                    <Link
                        ...
                        className="
                            ...
                            text-slate-300 rounded-lg hover:bg-slate-700/50 hover:border-slate-600/50 
                            hover:text-white hover:scale-105 transition-all duration-300 group
                        "
                    >
                ...
                <div className="
                    ...
                    rounded-3xl shadow-2xl shadow-cyan-500/10 overflow-hidden
                    ...
                    <div className="relative z-10 grid lg:grid-cols-2 gap-8 lg:gap-12 p-6 lg:p-12">
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="space-y-4">
                                ...
                                ">
                                    ...
                                    <div className="flex justify-between items-center py-2 border-b border-slate-600/30">
                                        ...
                                        <span className="text-slate-200">{getProductCategory(item.name)}</span>
                                    </div>
                                    <div className="flex justify-between items-center py-2 border-b border-slate-600/30">
                            ...
                            <div className="flex flex-col sm:flex-row gap-4">
                                <div className="
                                    flex-1 px-6 py-4 bg-gradient-to-r from-cyan-500 to-blue-600 
                                ...
                                <div className="
                                    px-6 py-4 bg-slate-700/50 border border-slate-600/50 
                                    text-slate-300 font-semibold rounded-xl backdrop-blur-sm
                                    hover:bg-slate-600/50 hover:border-slate-500/50 hover:text-white
                                    hover:scale-105 transition-all duration-300 text-center cursor-pointer
                                ">
                                    <div className="flex items-center justify-center space-x-2">
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function getProductFeatures(item) {
    ...
    if (name.includes('earbuds') || name.includes('headphones')) {
        features.push('Active Noise Cancellation', 'Wireless Connectivity', 'Long Battery Life', 'Premium Sound Quality');
    } else if (name.includes('pillow')) {
        features.push('Memory Foam Construction', 'Ergonomic Design', 'Travel-Friendly Size', 'Machine Washable Cover');
    } else if (name.includes('vacuum') || name.includes('robot')) {
        features.push('Smart Navigation', 'Auto-Charging', 'App Control', 'Multi-Surface Cleaning');
    } else if (name.includes('tracker') || name.includes('fitness')) {
        features.push('Heart Rate Monitor', 'Sleep Tracking', 'Water Resistant', 'Long Battery Life');
    } else if (name.includes('projector')) {
        features.push('HD Resolution', 'Portable Design', 'Multiple Connectivity', 'Built-in Speakers');
    } else if (name.includes('plug') || name.includes('smart')) {
        features.push('Voice Control', 'Remote Access', 'Energy Monitoring', 'Easy Setup');
    } else if (name.includes('kettle')) {
        features.push('Fast Boiling', 'Auto Shut-off', 'Temperature Control', 'Stainless Steel');
    } else if (name.includes('backpack')) {
        features.push('Multiple Compartments', 'Durable Material', 'Comfortable Straps', 'Water Resistant');
    } else {
        features.push('High Quality Materials', 'Durable Construction', 'Modern Design', 'Great Value');
    }
...
function getProductCategory(name) {
    ...
    if (nameLower.includes('earbuds') || nameLower.includes('headphones')) return 'Audio & Electronics';
    if (nameLower.includes('pillow')) return 'Home & Comfort';
    if (nameLower.includes('vacuum') || nameLower.includes('robot')) return 'Home Appliances';
    if (nameLower.includes('tracker') || nameLower.includes('fitness')) return 'Health & Fitness';
    if (nameLower.includes('projector')) return 'Electronics';
    if (nameLower.includes('plug') || nameLower.includes('smart')) return 'Smart Home';
    ...
    if (nameLower.includes('cream') || nameLower.includes('beauty')) return 'Beauty & Personal Care';
